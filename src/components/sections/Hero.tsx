...
import {
  Shield, Activity, Lock, Network, Terminal as TerminalIcon, FileText,
...
export function HeroSection() {
  return (
    <section id="home" className="relative pt-24 pb-10 px-4 sm:px-6 sweep-line">
      {/* Floating particles */}
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid lg:grid-cols-5 gap-8 items-stretch">
          ...
          >
            ...
            <div className="relative">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <a href="#projects" className="group inline-flex items-center gap-2 rounded-md px-5 py-2.5 font-mono text-sm text-primary-foreground glow-cyber transition-transform hover:-translate-y-0.5" style={{ background: "var(--gradient-cyber)" }}>
                  <TerminalIcon className="h-4 w-4" /> ./view_projects <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
                </a>
                <a href="#contact" className="inline-flex items-center gap-2 rounded-md border border-primary/40 bg-primary/5 px-5 py-2.5 font-mono text-sm text-primary hover:bg-primary/10 transition-colors">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="lg:col-span-2 panel p-5 flex flex-col min-w-0">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2 font-mono text-xs text-muted-foreground"><Activity className="h-4 w-4 text-accent" /> SECURITY · STATUS</div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 text-xs font-mono">
              {[["Firewall", "active"], ["VPN", "encrypted"], ["IPS", "monitoring"], ["Logs", "streaming"]].map(([k, v]) => (
                <div key={k} className="flex items-center justify-between rounded border border-border/60 bg-background/40 px-3 py-2">
                  <span className="text-muted-foreground">{k}</span>
                  <span className="text-accent inline-flex items-center gap-1"><CheckCircle2 className="h-3 w-3" /> {v}</span>
                </div>
              ))}
            </div>
          </motion.div>
        ...
        <div className="mt-10 grid grid-cols-2 lg:grid-cols-4 gap-3">
          {stats.map((s, i) => (
            <motion.div key={s.label} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }} className="panel p-4 hover:border-primary/40 transition-colors">
              <div className="flex items-center justify-between">
                <span className="font-mono text-[10px] text-muted-foreground tracking-widest">{s.label}</span>
                <s.icon className="h-4 w-4 text-primary" />
              </div>
              <div className="mt-2 text-2xl font-bold text-glow-cyber text-primary glow-pulse">{s.value}</div>
        ...
        {/* Quick Skills */}
        <div className="mt-20">
          <SectionHeading tag="modules.core" title="Security Modules" description="Three pillars driving every engagement — defense, routing, and tooling." icon={<Cpu className="h-3.5 w-3.5" />} />
          <div className="grid md:grid-cols-3 gap-4">
            {quickSkills.map((s, i) => (
              <motion.div key={s.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.08 }} className="panel p-6 group hover:border-primary/50 transition-all hover:-translate-y-1">
