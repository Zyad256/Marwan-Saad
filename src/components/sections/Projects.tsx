...
function Node({ label, sub, x, y, accent = false }: { label: string; sub?: string; x: string; y: string; accent?: boolean }) {
  return (
    <div className="absolute -translate-x-1/2 -translate-y-1/2 text-center" style={{ left: x, top: y }}>
      <div className={`mx-auto h-10 w-10 rounded-md flex items-center justify-center border ${accent ? "border-accent/60 bg-accent/10 text-accent glow-matrix" : "border-primary/50 bg-primary/10 text-primary"}`}>
...
function Topology() {
  return (
    <div className="relative h-72 w-full rounded-md border border-border/60 bg-background/40 overflow-hidden">
      <div className="absolute inset-0 opacity-40" style={{ backgroundImage: "linear-gradient(var(--grid-line) 1px, transparent 1px), linear-gradient(90deg, var(--grid-line) 1px, transparent 1px)", backgroundSize: "24px 24px" }} />
      <svg className="absolute inset-0 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none">
        {[["50,15", "25,45"], ["50,15", "75,45"], ["25,45", "15,80"], ["25,45", "40,80"], ["75,45", "60,80"], ["75,45", "85,80"], ["25,45", "75,45"]].map(([a, b], i) => {
          const [x1, y1] = a.split(",").map(Number);
          const [x2, y2] = b.split(",").map(Number);
          return (
            <g key={i}>
              <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="oklch(0.82 0.17 195 / 0.5)" strokeWidth="0.3" strokeDasharray="0.8 0.6" />
              <circle r="0.8" fill="oklch(0.85 0.20 145)" opacity="0.9">
                <animate attributeName="cx" values={`${x1};${x2};${x1}`} dur={`${3 + i * 0.7}s`} repeatCount="indefinite" />
                <animate attributeName="cy" values={`${y1};${y2};${y1}`} dur={`${3 + i * 0.7}s`} repeatCount="indefinite" />
              </circle>
            </g>
          );
        })}
  ...
  return (
    <section id="projects" className="py-20 px-4 sm:px-6">
      <div className="max-w-7xl mx-auto">
        <SectionHeading tag="projects.case_study" title="Engineering Case Studies" description="Real lab builds documented like production engagements." icon={<Building2 className="h-3.5 w-3.5" />} />

        <motion.article initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="panel-glow p-6 sm:p-8 relative overflow-hidden">
          <div className="absolute inset-0 opacity-25 pointer-events-none" style={{ background: "radial-gradient(700px circle at 100% 0%, oklch(0.85 0.20 145 / 0.18), transparent 60%)" }} />
          <div className="relative">
            <div className="flex items-center gap-2 chip mb-4"><Building2 className="h-3 w-3" /> CAPSTONE · ENTERPRISE</div>
            <h2 className="text-2xl sm:text-3xl font-bold">Hotel Network Infrastructure</h2>
            <p className="mt-2 text-muted-foreground max-w-3xl">End-to-end design of a multi-VLAN, multi-protocol hotel network with redundant gateways, encrypted branch connectivity, and a hardened management plane.</p>

            <div className="mt-6 grid lg:grid-cols-5 gap-5">
              <div className="lg:col-span-3 panel p-5 min-w-0"><Topology /></div>
              <div className="lg:col-span-2 space-y-2">
                {phases.map((p, i) => (
                  <motion.div key={p.tag} initial={{ opacity: 0, x: 16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }} className="panel p-4">
              ...
              {features.map((f) => (
                <div key={f.title} className="panel p-4 hover:border-primary/40 transition-colors">
                  <div className="flex items-start gap-3">
                    <div className="h-9 w-9 rounded-md border border-primary/40 bg-primary/10 flex items-center justify-center text-primary shrink-0"><f.icon className="h-4 w-4" /></div>
                    <div><div className="font-semibold text-sm">{f.title}</div><div className="text-xs text-muted-foreground mt-0.5">{f.desc}</div></div>
                  </div>
                </div>
              ))}
            </div>
