          ...
          {groups.map((g, gi) => (
            <motion.div key={g.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45, delay: gi * 0.08 }} className="panel p-6">
              <div className="flex items-center gap-3 mb-5">
                <div className="h-10 w-10 rounded-md border border-primary/40 bg-primary/10 flex items-center justify-center text-primary"><g.icon className="h-5 w-5" /></div>
                <div>
                ...
                {g.skills.map((s) => (
                  <div key={s.name}>
                    ...
                    <div className="h-1.5 w-full rounded-full bg-background/60 border border-border/60 overflow-hidden">
                      <motion.div initial={{ width: 0 }} whileInView={{ width: `${s.level}%` }} viewport={{ once: true }} transition={{ duration: 0.9, ease: "easeOut" }} className="h-full rounded-full" style={{ background: g.accent === "matrix" ? "linear-gradient(90deg, var(--matrix), var(--cyber))" : "var(--gradient-cyber)" }} />
                    </div>
                  </div>
                ))}
              </div>
