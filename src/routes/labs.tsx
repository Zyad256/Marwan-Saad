...
const labs = [
  { icon: Lock, name: "IPsec VPN Lab", env: "Cisco ↔ FortiGate", status: "stable",
    desc: "Phase 1/2 negotiation, NAT-T, dead peer detection, dual-stack policy.",
    tags: ["IKEv2", "AES-256", "SHA-256"] },
  { icon: Lock, name: "SSL VPN Remote Access", env: "FortiGate", status: "stable",
    desc: "Portal + tunnel mode, MFA, split tunneling, granular auth groups.",
    tags: ["SSL", "MFA", "RBAC"] },
  { icon: ShieldAlert, name: "Firewall Rule Design & Testing", env: "FortiGate 7.4", status: "stable",
    desc: "Zero-trust policy stack, app-control, web filter profile chaining.",
    tags: ["DLP", "AppCtrl", "WebFilter"] },
  { icon: Activity, name: "Traffic Analysis", env: "Wireshark", status: "monitoring",
    desc: "Capture + decode TLS, ARP spoofing detection, throughput baselines.",
    tags: ["pcap", "TLS", "ARP"] },
  { icon: Bug, name: "IPS / Attack Simulation", env: "Kali → FortiGate", status: "armed",
    desc: "Nmap scans, exploit attempts blocked + signature tuning workflow.",
    tags: ["Nmap", "IPS", "SigTune"] },
  { icon: Wrench, name: "Troubleshooting Lab", env: "GNS3 / EVE-NG", status: "stable",
    desc: "Layered fault isolation across L1–L4, routing convergence drills.",
    tags: ["OSPF", "BGP", "ACL"] },
];
...
function LabsPage() {
  return (
    <PageShell>
      <SectionHeading
        tag="labs.environment"
        title="Security Operations Lab Environment"
        description="Continuously running scenarios that mirror production SOC workflows — designed to break, detect, and defend."
      ...
      <div className="panel p-4 flex flex-wrap items-center gap-4 mb-8 font-mono text-xs">
        <div className="flex items-center gap-2">
          ...
          <span className="text-accent">LAB FABRIC ONLINE</span>
        </div>
        ...
        <span className="opacity-40">|</span>
        <span className="text-muted-foreground">ips_signatures: <span className="text-primary">11k loaded</span></span>
        <span className="ml-auto text-muted-foreground">last_run: <span className="text-foreground">2m ago</span></span>
      </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {labs.map((l, i) => (
          <motion.div
            ...
            className="panel p-5 hover:border-primary/50 hover:-translate-y-1 transition-all group"
          >
            <div className="flex items-start justify-between">
              <div className="h-10 w-10 rounded-md border border-primary/40 bg-primary/10 flex items-center justify-center text-primary group-hover:glow-cyber transition-shadow">
      ...
      <div className="mt-20">
        <SectionHeading
          ...
          title="How I Work"
          description="Repeatable, layer-aware methodology applied to every incident or design problem."
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          <div className="grid sm:grid-cols-2 gap-3">
            {[
              "Identify OSI layer issue",
              "Capture traffic (Wireshark)",
              "Validate routing & connectivity",
              "Check firewall rules",
              "Analyze logs & security policies",
              "Apply fix & verify security posture",
            ].map((step, i) => (
              <motion.div
                key={step}
